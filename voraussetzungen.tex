...
\begin{adjustwidth}{-2.5em}{-2.5em}
	\begin{center}
		...
		[
			...
			box/.style = {basenode, minimum width=2.5cm, top color=white, bottom color=lightblue},
			centerbox/.style = {box, minimum width=2.5cm, minimum height=2.5cm},
			backbox/.style = {basenode, fill=lightblue!50, inner sep = .5cm},
		...
		\visible<4->{
			\node[above of = center] (floss) {FLOSS: Free/Libre Open Source Software};
		}

		\visible<1->{
			\node[centerbox, left of = center] (free) {\includegraphics[height=1.75cm]{res/gnu-head.pdf}};
		}
		\visible<2->{
			\node[box, below of = free] (user) {Anwender};
			\draw[property] (free) -- (user);
		}

		\visible<1->{
			\node[centerbox, right of = center] (open) {\includegraphics[height=1.75cm]{res/opensource.pdf}};
		}
		\visible<3->{
			\node[box, below of = open] (developer) {Entwickler};
			\draw[property] (open) -- (developer);
		}

		\begin{pgfonlayer}{background} 
		\visible<4->{
			\node[backbox] [fit = (floss) (free) (open) (user) (developer)] {};
		}
		\end{pgfonlayer}
